...
def run_pdf(
    ...
    *,
    ...
    try:
        ...
        try:
            models = model_report or ensure_model_artifacts(config.docling)
        ...
        backend = backend or DoclingBackend.from_config(
        ...
        from .visualization import render_layout_overlays

        overlays = render_layout_overlays(result, save=True)
        ...
        return PipelineRunSummary(
            ...
            document.artifacts.artifact_manifest_json,
            validation,
            tuple(overlay.path for overlay in overlays if overlay.path is not None),
        )
...
def _resume_summary(document: Any, config: PipelineConfig) -> PipelineRunSummary:
    ...
    return PipelineRunSummary(
        ...
        tuple(
            sorted(
                document.artifacts.overlay_dir.glob(
                    "page_*_docling_layout_overlay.png"
                )
            )
        ),
    )
