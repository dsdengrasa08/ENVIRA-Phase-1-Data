...
def export_pipeline_result(run):
    ...
    overlay_files = tuple(sorted(paths.overlay_dir.glob("page_*_docling_layout_overlay.png")))
    files = (
        paths.raw_json,
        paths.raw_markdown,
        paths.effective_config_json,
    ...
    manifest = {
        ...
        "original_filename": getattr(run.document, "original_name", None),
        "source_pdf_bytes": (
        ...
        "files": [
            {
                "path": path.relative_to(paths.document_dir).as_posix(),
