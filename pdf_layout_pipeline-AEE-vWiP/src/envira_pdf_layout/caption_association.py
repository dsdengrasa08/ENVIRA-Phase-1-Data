...
_REFERENCE_RE = re.compile(
    r"^\s*(?P<label>(?P<kind>fig(?:ure)?\.?|table|tab\.?|equation|eq\.?|"
    r"algorithm|listing)\s+(?P<number>[A-Z]?\d+(?:[.\-]\w+)?|[IVXLCDM]+))"
    r"(?:\s*[:.\-])?(?:\s+|$)",
    re.IGNORECASE,
)
...
def parse_caption_reference(text: Any) -> CaptionReference | None:
    ...
    kind = (
        "figure" if raw_kind in {"fig", "figure"}
        else "table" if raw_kind in {"tab", "table"}
        else "equation" if raw_kind in {"eq", "equation"}
        else raw_kind
    )
...
def _blocker_between(
    caption: LayoutRegion, parent: LayoutRegion, regions: list[LayoutRegion]
) -> str | None:
    cb, pb = caption["bbox_px"], parent["bbox_px"]
    top, bottom = sorted(((cb[1] + cb[3]) / 2, (pb[1] + pb[3]) / 2))
    for region in regions:
        if region is caption or region is parent or region.get("type") not in _BLOCKER_TYPES:
            continue
        rb = region["bbox_px"]
        center = (rb[1] + rb[3]) / 2
        if top < center < bottom and _horizontal_overlap(cb, rb) >= 0.25:
...
def associate_captions(
    ...
    *,
    ...
    min_horizontal_overlap: float = 0.18,
) -> list[dict[str, Any]]:
    ...
    for page_number, page_regions in sorted(by_page.items()):
        ...
        for caption in candidates:
            reference = parse_caption_reference(caption.get("text") or caption.get("orig"))
            ...
            for parent in parents:
                ...
                if overlap < min_horizontal_overlap or gap > max_vertical_gap_page_ratio:
                    continue
                caption_column, parent_column = (
                    caption.get("reading_order_column"), parent.get("reading_order_column")
                )
                if caption_column not in {None, "single", parent_column} and parent_column not in {None, "single"}:
                    continue
                blocker = _blocker_between(caption, parent, page_regions)
                if blocker:
                    continue
                direction_match = cb[1] >= pb[3] if parent.get("type") != "Table" else cb[3] <= pb[1]
                score = (
                    0.45 * overlap
                    + 0.30 * max(0.0, 1 - gap / max_vertical_gap_page_ratio)
                    + 0.15 * bool(reference)
            ...
            alternatives.sort(key=lambda item: (-item["score"], item["parent_region_id"]))
            if not alternatives:
                output.append({
                    "relationship_id": f"p{page_number}:caption:{caption['layout_region_id']}",
                    "page_number": page_number, "kind": "CAPTION_OF",
                    "child_region_id": str(caption["layout_region_id"]),
                    "parent_region_id": None, "candidate_parents": [],
                    "status": "no_compatible_parent", "proposed_action": "retain_unattached",
                    "caption_reference": reference.__dict__ if reference else None,
                })
                continue
            margin = alternatives[0]["score"] - alternatives[1]["score"] if len(alternatives) > 1 else None
            ambiguous = margin is not None and margin < ambiguity_margin
            output.append({
                "relationship_id": f"p{page_number}:caption:{caption['layout_region_id']}",
                "page_number": page_number, "kind": "CAPTION_OF",
                "child_region_id": str(caption["layout_region_id"]),
                "parent_region_id": None if ambiguous else alternatives[0]["parent_region_id"],
                ...
                "ambiguity_margin": round(margin, 6) if margin is not None else None,
                "caption_reference": reference.__dict__ if reference else None,
