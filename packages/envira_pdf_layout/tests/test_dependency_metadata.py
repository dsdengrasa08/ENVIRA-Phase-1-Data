...
def test_legacy_requirements_are_independent_of_callers_working_directory():
    ...
    assert not any(requirement.lower().startswith("pytest") for requirement in requirements)
...
def test_regression_direct_pins_satisfy_project_contract():
    ...
    pins = {}
    for line in (ROOT.parents[1] / "requirements" / "constraints-regression.txt").read_text(encoding="utf-8").splitlines():
