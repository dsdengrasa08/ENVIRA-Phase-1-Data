...
Open [`notebooks/pipeline_workflow_colab.ipynb`](../../notebooks/pipeline_workflow_colab.ipynb), set `SOURCE_PDF`, select the YAML
...
jupyter lab pipeline_workflow_colab.ipynb
```

From the repository root, use
`python -m pip install -e "./packages/envira_pdf_layout[notebook]"` instead.
Legacy and Colab callers may continue using the path-independent
`python -m pip install -r packages/envira_pdf_layout/requirements.txt` command.
...
untrusted-input deployment model are documented in [security guide](../../docs/reference/pipeline-security.md).
Dependency locking, clean-wheel verification, model manifests, SBOMs, and runtime
fingerprints are documented in [supply-chain guide](../../docs/operations/supply-chain.md).
Lifecycle events, cancellation, failure reports, preflight checks, metrics, and
service-level indicators are documented in [observability guide](../../docs/operations/observability.md).
...
- `preserved_core.py`: frozen compatibility extraction of the remaining established
  ...
  immutable contracts. See [core migration guide](../../docs/architecture/core-migration.md).
...
- `schema.py`: versioned region/relationship contracts, coordinate metadata,
  ...
  [schema guide](../../docs/reference/schemas.md) and the machine-readable contracts under `schemas/`.
...
`../../requirements/constraints-regression.txt` for reproducible model-backed or scheduled corpus runs;
...
The workflow notebook is pinned to `packages/envira_pdf_layout/src` and verifies the
...
[`heuristics guide`](../../docs/methodology/heuristics.md) for the active destructive-rule inventory and safeguards.
