...
CONTAINER_CAPABLE = ASSET_FAMILY | {"List"}
...
def overlap_features(a: LayoutRegion, b: LayoutRegion, page: dict[str, Any]) -> dict[str, Any]:
    ...
    if ta and tb:
        text_relation = "equal" if ta == tb else "a_in_b" if ta in tb else "b_in_a" if tb in ta else "different"
    else:
    ...
    return {
        ...
        "intersection_over_smaller": intersection / min(area_a, area_b) if min(area_a, area_b) else 0.0,
        "intersection_over_larger": intersection / max(area_a, area_b) if max(area_a, area_b) else 0.0,
        ...
        "area_ratio": min(area_a, area_b) / max(area_a, area_b) if max(area_a, area_b) else 0.0,
        ...
        "center_distance_page_ratio": (((ac[0] - bc[0]) / width) ** 2 + ((ac[1] - bc[1]) / height) ** 2) ** 0.5,
        "edge_delta_page_ratio": max(abs(ab[0]-bb[0])/width, abs(ab[2]-bb[2])/width, abs(ab[1]-bb[1])/height, abs(ab[3]-bb[3])/height),
        "horizontal_gap_page_ratio": max(0.0, max(ab[0], bb[0]) - min(ab[2], bb[2])) / width,
        "vertical_gap_page_ratio": max(0.0, max(ab[1], bb[1]) - min(ab[3], bb[3])) / height,
...
def _candidate_pairs(regions: list[LayoutRegion], page_height: float, near_ratio: float) -> Iterable[tuple[LayoutRegion, LayoutRegion]]:
    """Sweep-line candidate generation for intersecting and nearby fragments."""
    margin = max(0.0, page_height * near_ratio)
    ordered = sorted(regions, key=lambda r: (float(r["bbox_px"][1]), float(r["bbox_px"][0])))
...
def _compatible_duplicate_roles(a: LayoutRegion, b: LayoutRegion, f: dict[str, Any]) -> bool:
    if f["same_class"]:
        return True
...
def _classify(a: LayoutRegion, b: LayoutRegion, f: dict[str, Any], config: OverlapResolutionConfig) -> tuple[str, str, str]:
    duplicate_geometry = (
    ...
    if duplicate_geometry and _compatible_duplicate_roles(a, b, f) and f["text_relation"] in {"equal", "unavailable"}:
    ...
    if f["intersection_over_smaller"] >= config.nested_containment:
        return "LEGITIMATE_CONTAINMENT", "directional_containment", "associate"
    if f["intersection_area"] > 0:
        families = {f["left_family"], f["right_family"]}
        smaller_h = min(float(a["bbox_px"][3])-float(a["bbox_px"][1]), float(b["bbox_px"][3])-float(b["bbox_px"][1]))
        penetration = min(max(0.0, float(a["bbox_px"][3])-float(b["bbox_px"][1])), max(0.0, float(b["bbox_px"][3])-float(a["bbox_px"][1])))
        if "asset" in families and penetration / max(1.0, smaller_h) <= config.boundary_overlap_ratio:
...
def resolve_layout_overlaps(regions, pages, config: OverlapResolutionConfig | None = None) -> ResolutionResult:
    ...
    for page_number, page_regions in by_page.items():
        ...
        for a, b in _candidate_pairs(page_regions, height, config.fragment_max_gap_page_ratio):
            f = overlap_features(a, b, page)
            kind, reason, proposed_action = _classify(a, b, f, config)
            if kind == "INDEPENDENT":
                continue
            left, right = str(a["layout_region_id"]), str(b["layout_region_id"])
            relation: dict[str, Any] = {
                "relationship_id": f"p{page_number}:{left}:{right}",
                "left_region_id": left, "right_region_id": right,
                "page_number": page_number, "kind": kind, "reason": reason,
                "proposed_action": proposed_action, "status": "observed",
                "features": f, "left_class": a.get("type"), "right_class": b.get("type"),
                "left_raw_class": a.get("docling_label"), "right_raw_class": b.get("docling_label"),
                "left_score": a.get("score"), "right_score": b.get("score"),
                "left_bbox": list(a["bbox_px"]), "right_bbox": list(b["bbox_px"]),
            }
            if kind == "DUPLICATE":
                duplicate_edges.append((left, right))
            elif kind == "LEGITIMATE_CONTAINMENT":
                a_inside = f["a_containment"] >= f["b_containment"]
                relation["parent_region_id"] = right if a_inside else left
                relation["child_region_id"] = left if a_inside else right
                relation["status"] = "preserved_as_parent_child"
    ...
    # A non-container detection that substantially covers several otherwise
    # independent regions is more plausibly an oversized false positive than a
    # legitimate parent.  Flag it globally instead of deleting either side.
    contained_by: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for relation in relationships:
        if relation["kind"] == "LEGITIMATE_CONTAINMENT":
            contained_by[str(relation["parent_region_id"])].append(relation)
    for parent_id, relations in contained_by.items():
        parent_region = by_id[parent_id]
        if parent_region.get("type") in CONTAINER_CAPABLE or len(relations) < 2:
            continue
        for relation in relations:
            relation.update(
                kind="INVALID_OCCLUSION",
                reason="non_container_covers_multiple_regions",
                proposed_action="flag",
                status="unresolved_conflict",
            )
            relation.pop("parent_region_id", None)
            relation.pop("child_region_id", None)
    ...
    for members in components.values():
        if len(members) > 1 and all(frozenset((a, b)) in duplicate_pairs for i, a in enumerate(members) for b in members[i+1:]):
            accepted_components.append(members)
    source_order = {str(r["layout_region_id"]): i for i, r in enumerate(working)}
    def quality(rid):
        r = by_id[rid]
        score = r.get("score")
        return (bool(_text(r)), len(_normalized_text(r)), score is not None, float(score) if score is not None else 0.0, -source_order[rid])
    ...
    for members in accepted_components:
        ...
        decisions.append({"action": "canonicalize", "canonical_region_id": canonical, "source_region_ids": sorted(members), "reason": "complete_link_duplicate_component", "confidence": "high"})
    ...
    for rel in relationships:
        ...
        if rel["kind"] == "DUPLICATE":
            canonical = duplicate_of.get(rel["left_region_id"], duplicate_of.get(rel["right_region_id"]))
            if canonical:
                rel.update(status="collapsed", canonical_region_id=canonical)
            else:
                rel.update(status="retained_nontransitive_duplicate_chain", proposed_action="flag")
    output = []
    for region in working:
        rid = str(region["layout_region_id"])
        if rid in duplicate_of:
            continue
        sources = [rid] + sorted(member for member, canonical in duplicate_of.items() if canonical == rid)
        region["source_region_ids"] = list(dict.fromkeys(region["source_region_ids"] + sources))
        if len(sources) > 1:
            region["resolution_action"] = "clear_duplicate_collapsed"
        parents = sorted({rel["parent_region_id"] for rel in relation_by_id[rid] if rel.get("child_region_id") == rid})
        children = sorted({rel["child_region_id"] for rel in relation_by_id[rid] if rel.get("parent_region_id") == rid})
        conflicts = sorted(rel["relationship_id"] for rel in relation_by_id[rid] if rel["status"] == "unresolved_conflict")
        if parents:
            region["nested_parent_region_ids"] = parents
            region["emission_policy"] = "emit_as_nested_child"
        if children:
            region["nested_child_region_ids"] = children
        if conflicts:
    ...
    # Stable final top-level order; children receive a parent-local order and do
    # not perturb the page-level sequence.
    ...
    for page_regions in out_pages.values():
        page_regions.sort(key=lambda r: (int(r.get("layout_reading_order") or 10**9), float(r["bbox_px"][1]), float(r["bbox_px"][0])))
        top_order = 0
        child_orders: dict[str, int] = defaultdict(int)
        for region in page_regions:
            parents = region.get("nested_parent_region_ids", [])
            if parents:
                child_orders[parents[0]] += 1
                region["parent_local_reading_order"] = child_orders[parents[0]]
                region["resolved_reading_order"] = None
            else:
                top_order += 1
...
def associate_attachable_context(regions, pages, *, ambiguity_margin: float = 0.15) -> list[dict[str, Any]]:
    ...
    for page_number, page_regions in by_page.items():
        ...
        for candidate in candidates:
            ...
            ambiguous = len(alternatives) > 1 and winner_score - alternatives[1][0] < ambiguity_margin
            associations.append({
                "relationship_id": f"p{page_number}:context:{candidate['layout_region_id']}",
                "page_number": page_number,
                "kind": "CAPTION_OF",
                "child_region_id": str(candidate["layout_region_id"]),
                "parent_region_id": None if ambiguous else str(winner["layout_region_id"]),
                "candidate_parent_region_ids": [str(parent["layout_region_id"]) for _, parent in alternatives],
                "confidence": round(winner_score, 4),
                "ambiguity_margin": round(winner_score - alternatives[1][0], 4) if len(alternatives) > 1 else None,
                "status": "unresolved_conflict" if ambiguous else "associated",
                "proposed_action": "flag" if ambiguous else "associate",
                "features": {"page_width": width, "alternative_count": len(alternatives)},
            })
    return associations
